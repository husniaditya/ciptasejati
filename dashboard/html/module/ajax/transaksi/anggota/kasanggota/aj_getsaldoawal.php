...
if (isset($_POST["KAS_ID"])) {
    ...
    $getSaldoAwal = GetQuery("SELECT FORMAT(IFNULL(SUM(KAS_JUMLAH),0), 0) AS SALDOAWAL FROM t_kas WHERE DELETION_STATUS = 0 AND ANGGOTA_KEY = '$ANGGOTA_KEY' AND KAS_JENIS = '$KAS_JENIS' AND KAS_TANGGAL <= (SELECT MIN(KAS_TANGGAL) FROM t_kas WHERE DELETION_STATUS = 0 AND ANGGOTA_KEY = '$ANGGOTA_KEY' and KAS_JENIS = '$KAS_JENIS')");
