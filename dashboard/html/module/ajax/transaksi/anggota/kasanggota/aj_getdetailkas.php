...
$getDetail = GetQuery("SELECT k.*,d.DAERAH_KEY,c.CABANG_KEY,d.DAERAH_DESKRIPSI,c.CABANG_DESKRIPSI,a.ANGGOTA_ID,a.ANGGOTA_NAMA,t.TINGKATAN_NAMA,t.TINGKATAN_SEBUTAN,a2.ANGGOTA_NAMA INPUT_BY,DATE_FORMAT(k.KAS_TANGGAL, '%d %M %Y') FKAS_TANGGAL, DATE_FORMAT(k.INPUT_DATE, '%d %M %Y %H:%i') INPUT_DATE, FORMAT(ABS(REPLACE(k.KAS_JUMLAH,'-','')), 0) KAS_JUMLAH,CONCAT(a.ANGGOTA_ID,' - ',a.ANGGOTA_NAMA) AS ANGGOTA_IDNAMA,
CASE
...
WHERE k.DELETION_STATUS = 0 AND a.DELETION_STATUS=0 and k.KAS_ID = '$KAS_ID'
ORDER BY k.KAS_ID");
