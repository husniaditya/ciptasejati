...
if (isset($_POST["DAERAH_KEY"]) || isset($_POST["CABANG_KEY"]) || isset($_POST["TINGKATAN_ID"]) || isset($_POST["ANGGOTA_ID"]) || isset($_POST["ANGGOTA_KTP"]) || isset($_POST["ANGGOTA_NAMA"]) || isset($_POST["ANGGOTA_HP"]) || isset($_POST["ANGGOTA_STATUS"])) {
    ...
    if ($USER_AKSES == "Administrator") {
        ...
        LEFT JOIN m_daerah d ON c.DAERAH_KEY = d.DAERAH_KEY
        LEFT JOIN m_tingkatan t ON a.TINGKATAN_ID = t.TINGKATAN_ID
        WHERE a.CABANG_KEY = '$USER_CABANG' AND (a.TINGKATAN_ID LIKE CONCAT('%','$TINGKATAN_ID','%')) AND (a.ANGGOTA_KTP LIKE CONCAT('%','$ANGGOTA_KTP','%')) AND (a.ANGGOTA_NAMA LIKE CONCAT('%','$ANGGOTA_NAMA','%')) AND (a.ANGGOTA_HP LIKE CONCAT('%','$ANGGOTA_HP','%')) AND (a.ANGGOTA_STATUS LIKE CONCAT('%','$ANGGOTA_STATUS','%')) and (a.ANGGOTA_ID LIKE CONCAT('%','$ANGGOTA_ID','%'))");
    }
} else {
    if ($USER_AKSES == "Administrator") {
        $getAnggotadata = GetQuery("SELECT a.*,d.DAERAH_KEY,d.DAERAH_DESKRIPSI,c.CABANG_DESKRIPSI,t.TINGKATAN_NAMA,t.TINGKATAN_GELAR,t.TINGKATAN_SEBUTAN,DATE_FORMAT(a.ANGGOTA_TANGGAL_LAHIR, '%d %M %Y') TGL_LAHIR,DATE_FORMAT(a.ANGGOTA_JOIN, '%d %M %Y') TGL_JOIN,DATE_FORMAT(a.ANGGOTA_RESIGN, '%d %M %Y') TGL_RESIGN,RIGHT(a.ANGGOTA_ID,3) SHORT_ID, CASE WHEN ANGGOTA_STATUS = 0 THEN 'Aktif' WHEN ANGGOTA_STATUS = 1 THEN 'Non Aktif' ELSE 'Mutasi' END STATUS_DES FROM m_anggota a
