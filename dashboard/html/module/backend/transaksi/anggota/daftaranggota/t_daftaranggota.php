...
if (isset($_POST["editdaftaranggota"])) {
    ...
    try {
        ...
        if ($ANGGOTA_STATUS == 0) {
            // Prepare the query
            $query = "UPDATE m_anggota set CABANG_KEY = ?, ANGGOTA_RANTING = ?, TINGKATAN_ID = ?, ANGGOTA_KTP = ?, ANGGOTA_NAMA = ?, ANGGOTA_ALAMAT = ?, ANGGOTA_AGAMA = ?, ANGGOTA_PEKERJAAN = ?, ANGGOTA_KELAMIN = ?, ANGGOTA_TEMPAT_LAHIR = ?, ANGGOTA_TANGGAL_LAHIR = ?, ANGGOTA_HP = ?, ANGGOTA_EMAIL = ?, ANGGOTA_JOIN = ?, ANGGOTA_STATUS = ?, ANGGOTA_RESIGN = '', ANGGOTA_AKSES = ?, INPUT_BY = ?, INPUT_DATE = '$localDateTime' where ANGGOTA_KEY = ?";
