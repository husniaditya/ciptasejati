    ...
    try {
        ...
        while ($rowCheckID = $checkID->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($COUNT_ID > 0) {
                ...
                // Prepare the query
                $query = "INSERT INTO m_anggota SELECT UUID(), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, 0, 0, ?, NOW()";
...
if (isset($_POST["editdaftaranggota"])) {
    ...
    try {
        ...
        if ($ANGGOTA_STATUS == 0) {
            // Prepare the query
            $query = "UPDATE m_anggota set CABANG_KEY = ?, ANGGOTA_RANTING = ?, TINGKATAN_ID = ?, ANGGOTA_KTP = ?, ANGGOTA_NAMA = ?, ANGGOTA_ALAMAT = ?, ANGGOTA_AGAMA = ?, ANGGOTA_PEKERJAAN = ?, ANGGOTA_KELAMIN = ?, ANGGOTA_TEMPAT_LAHIR = ?, ANGGOTA_TANGGAL_LAHIR = ?, ANGGOTA_HP = ?, ANGGOTA_EMAIL = ?, ANGGOTA_JOIN = ?, ANGGOTA_AKSES = ?, INPUT_BY = ?, INPUT_DATE = now() where ANGGOTA_KEY = ?";

            // Execute the query with parameters
            GetQuery2($query, [$CABANG_KEY, $ANGGOTA_RANTING, $TINGKATAN_ID, $ANGGOTA_KTP, $ANGGOTA_NAMA, $ANGGOTA_ALAMAT, $ANGGOTA_AGAMA, $ANGGOTA_PEKERJAAN, $ANGGOTA_KELAMIN, $ANGGOTA_TEMPAT_LAHIR, $ANGGOTA_TANGGAL_LAHIR, $ANGGOTA_HP, $ANGGOTA_EMAIL, $ANGGOTA_JOIN, $ANGGOTA_AKSES, $USER_ID, $ANGGOTA_KEY]);
        } else {
            // Prepare the query
            $query = "UPDATE m_anggota set CABANG_KEY = ?, ANGGOTA_RANTING = ?, TINGKATAN_ID = ?, ANGGOTA_KTP = ?, ANGGOTA_NAMA = ?, ANGGOTA_ALAMAT = ?, ANGGOTA_AGAMA = ?, ANGGOTA_PEKERJAAN = ?, ANGGOTA_KELAMIN = ?, ANGGOTA_TEMPAT_LAHIR = ?, ANGGOTA_TANGGAL_LAHIR = ?, ANGGOTA_HP = ?, ANGGOTA_EMAIL = ?, ANGGOTA_JOIN = ?, ANGGOTA_AKSES = ?, INPUT_BY = ?, INPUT_DATE = now() where ANGGOTA_KEY = ?";
