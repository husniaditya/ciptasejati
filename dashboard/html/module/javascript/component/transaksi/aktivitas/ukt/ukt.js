// Helper to initialize DataTable
...
// Helper to reload DataTable with AJAX
function reloadTable(selector, url, target, callback) {
  $.ajax({
    type: 'POST',
    url,
...
function callTable() {
  ['#ukt-table', '#uktkoor-table', '#lapukt-table'].forEach(initDataTable);
  $('#ApprovePPDGuru, #ViewPPDGuru').on('shown.bs.modal', function () {
    ...
    initDataTable('#detailppd-table');
  });
}

// AJAX helpers
function savePDFToDrive(UKT_ID) {
  return $.post('module/backend/transaksi/aktivitas/ukt/t_uktfile.php', { id: UKT_ID });
}
function sendEmailNotification(id) {
  return $.post('module/backend/transaksi/anggota/mutasianggota/t_mutasimail.php', { id });
}

// Reset preview dropdowns
function resetPreview(modalId) {
  let ids = ['#selectize-dropdown9', '#selectize-dropdown10', '#selectize-dropdown11', '#selectize-dropdown12'];
  let isExist = ids.every(id => $(id).length > 0);
  if (isExist) $('#selectize-dropdown9')[0].selectize.clear();
  if (isExist) $('#selectize-dropdown10')[0].selectize.clear();
  $('#selectize-dropdown2')[0].selectize.clear();
  $('#selectize-dropdown4')[0].selectize.clear();
  $('#selectize-dropdown8')[0].selectize.clear();
  $('#selectize-dropdown13')[0].selectize.clear();
}

// Form handler factory
function handleFormGeneric(formId, url, reloadUrl, tableSelector, targetSelector, notification, closeModal = true, afterSuccess) {
  $(formId).submit(function (event) {
    event.preventDefault();
    let formData = new FormData(this);
    let buttonId = $(event.originalEvent.submitter).attr('id');
    formData.append(buttonId, 'edit');
    let showLoadingOverlay = msg => $('body').append(`<div id="loading-overlay" class="loading-overlay"><div class="loading-spinner"></div><div class="loading-message">${msg}</div><div class="progress-bar"><div class="progress"></div></div></div>`);
    let hideLoadingOverlay = () => $('#loading-overlay').remove();
    ...
    $.ajax({
      ...
      contentType: false,
      success: function (response) {
        let [successMessage, UKT_ID] = response.split(',');
        if (successMessage === 'Success') {
          notification('Data berhasil tersimpan!');
          if (closeModal) $(formId.replace("-form", "")).modal('hide');
          reloadTable(tableSelector, reloadUrl, targetSelector);
          if (afterSuccess) afterSuccess(UKT_ID);
        } else {
...
$(function () {
  callTable();
  handleFormGeneric('#AddUKT-form', 'module/backend/transaksi/aktivitas/ukt/t_ukt.php', 'module/ajax/transaksi/aktivitas/ukt/aj_tableukt.php', '#ukt-table', '#uktdata', SuccessNotification);
  handleFormGeneric('#EditUKT-form', 'module/backend/transaksi/aktivitas/ukt/t_ukt.php', 'module/ajax/transaksi/aktivitas/ukt/aj_tableukt.php', '#ukt-table', '#uktdata', UpdateNotification);
  handleFormGeneric('#ApproveUKTKoordinator-form', 'module/backend/transaksi/aktivitas/ukt/t_ukt.php', 'module/ajax/transaksi/aktivitas/ukt/aj_tableuktkoor.php', '#uktkoor-table', '#uktdatakoor', UpdateNotification, true, UKT_ID => {
    savePDFToDrive(UKT_ID).always(() => $('#loading-overlay').remove());
  });
  handleFormGeneric('#ApproveUKTGuru-form', 'module/backend/transaksi/aktivitas/ukt/t_ukt.php', 'module/ajax/transaksi/aktivitas/ukt/aj_tableuktguru.php', '#ukt-table', '#uktdataguru', UpdateNotification);

  // Selectize change events (simplified)
  $(document).on('change', '#selectize-select3', function () {
    let val = $(this).val();
    $.post('module/ajax/transaksi/anggota/daftaranggota/aj_getlistcabang.php', { id: val }, data => {
      let s2 = $('#selectize-select2')[0].selectize;
      s2.clearOptions();
      s2.addOption(data);
      s2.setValue('');
    }, 'json');
  ...
  $('#AddUKT').on('hide.bs.modal', function () {
    if ($(document.activeElement).closest(this).length) {
      $(document.activeElement).blur();
    }
  });

  $('#EditUKT').on('hide.bs.modal', function () {
    if ($(document.activeElement).closest(this).length) {
      $(document.activeElement).blur();
    }
  });

  $('#ViewUKT').on('hide.bs.modal', function () {
    if ($(document.activeElement).closest(this).length) {
      $(document.activeElement).blur();
    }
  });
