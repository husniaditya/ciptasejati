...
if (isset($_GET['id']) && isset($_GET['data']) && isset($_GET['pic'])) {
    ...
    $getData = GetQuery("SELECT u.UKT_ID DOKUMEN_ID,d.DAERAH_DESKRIPSI,c.CABANG_DESKRIPSI,a.ANGGOTA_NAMA, koor.ANGGOTA_NAMA KOOR_NAMA, guru.ANGGOTA_NAMA GURU_NAMA, date_format(u.INPUT_DATE, '%d-%m-%Y') DOKUMEN_DATE, date_format(u.INPUT_DATE, '%H:%i:%s') DOKUMEN_TIME, date_format('$localDateTime', '%d-%m-%Y') DATENOW, date_format('$localDateTime', '%H:%i:%s') TIMENOW
    FROM t_ukt u
